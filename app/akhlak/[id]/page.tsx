  ...
  return (
    <div className="h-screen w-screen overflow-hidden relative bg-slate-950 text-slate-100 flex flex-col font-sans select-none">
      ...
      <header className="relative z-10 w-full border-b border-white/10 bg-slate-950/60 backdrop-blur-md px-6 py-3 flex items-center justify-between shrink-0">
        ...
        <span className="text-xs font-semibold text-slate-300">Nilai: <strong className="text-white">{akhlakName}</strong></span>
      </header>
      ...
      <main className="relative z-10 flex-1 max-w-5xl mx-auto w-full px-6 py-8 flex flex-col justify-center overflow-y-auto custom-scrollbar">
        <div className="text-center mb-6">
          <h1 className="text-2xl font-extrabold text-white tracking-tight">{akhlakName}</h1>
          <p className="text-xs text-slate-400 mt-1">Daftar program inovasi yang mencerminkan nilai BerAKHLAK ini.</p>
        </div>

        {isLoading ? (
          <div className="flex justify-center items-center py-12 text-slate-400">
            <Loader2 className="w-6 h-6 animate-spin mr-2 text-[#0B5E90]" /> Memuat data...
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {inovasiList.map((item) => (
              <div 
                ...
                className="group bg-slate-900/90 hover:bg-slate-900 border border-white/10 hover:border-blue-500/50 rounded-xl p-4 cursor-pointer transition-all flex flex-col justify-between shadow-lg"
              >
                <div>
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <h3 className="text-xs font-bold text-white group-hover:text-blue-400 transition-colors line-clamp-1">{item.nama_inovasi}</h3>
                  </div>
                ...
                <div className="flex items-center justify-between text-[10px] font-bold text-blue-400 pt-2 border-t border-white/10">
          ...
          <div className="text-center py-10 bg-white/5 rounded-xl border border-white/10 max-w-sm mx-auto w-full">
            ...
            <p className="text-xs text-slate-400">Belum ada inovasi untuk nilai ini.</p>
          </div>
        )}
      </main>
    </div>
