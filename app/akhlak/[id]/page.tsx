  ...
  return (
    <div className="min-h-screen w-full bg-slate-50 text-slate-800 flex flex-col font-sans select-none overflow-x-hidden">
      ...
      <header className="sticky top-0 z-10 w-full bg-white border-b border-slate-200 px-6 py-4 flex items-center shadow-sm">
        <Link 
          href="/"
          className="inline-flex items-center gap-2 text-slate-500 hover:text-[#0B5E90] text-sm font-bold transition-colors"
        >
      ...
      <main className="flex-1 w-full max-w-6xl mx-auto px-6 py-12 flex flex-col items-center">
        ...
        <div className="text-center mb-10 w-full">
          <span className="inline-block px-3 py-1 bg-blue-100 text-[#0B5E90] text-[10px] font-extrabold uppercase tracking-widest rounded-full mb-3">
            Nilai BerAKHLAK
          </span>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 w-full">
            {inovasiList.map((item) => (
              <div 
                ...
                className="group flex flex-col items-center p-4 bg-white border border-slate-200 rounded-2xl cursor-pointer hover:border-[#0B5E90] hover:shadow-lg transition-all duration-300 hover:-translate-y-1"
              >
                <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center p-2 mb-4 overflow-hidden group-hover:bg-blue-50/50 transition-colors">
                  {item.logo ? (
                ...
                <h3 className="text-xs sm:text-sm font-bold text-slate-700 text-center line-clamp-2 group-hover:text-[#0B5E90] transition-colors">
                  {item.nama_inovasi}
                </h3>
              </div>
            ))}
          ...
          <div className="text-center py-20 w-full max-w-md bg-white border border-slate-200 border-dashed rounded-2xl mt-4">
            ...
            <p className="text-sm text-slate-500">Belum ada inovasi yang terdaftar pada nilai ini.</p>
          </div>
        )}
      </main>
    </div>
